...
int main() {
    ...
    SetConsoleCP(CP_UTF8);  // 입력도 한글로 받을 경우 필요
    ...
    while (isRunning) {
        if (!isLoggedIn) {
            ...
            switch (choice) {
            case 1: {
                std::cout << "채팅을 시작합니다. (exit 입력 시 종료)\n";
                while (true) {
                    std::cout << "> ";
                    std::string chat;
                    ...
                    if (chat == "exit") {
                        std::string response = sendAndReceive(sock, "exit");
                        std::cout << response << std::endl;
                        isLoggedIn = false;
                        break;
                    }

                    std::string response = sendAndReceive(sock, "CHAT:" + chat);
                    std::cout << response << std::endl;
                }
                break;
            }
